...
/** launchd Label + plist basename. e.g. `com.coda-ab12cd34`. */
export function getLaunchdLabel(projectRoot?: string): string {
  return `com.coda-${getInstallSlug(projectRoot)}`;
}

/** systemd unit name (no .service suffix). e.g. `coda-ab12cd34`. */
export function getSystemdUnit(projectRoot?: string): string {
  return `coda-${getInstallSlug(projectRoot)}`;
}

/** Docker image base (no tag). e.g. `coda-agent-ab12cd34`. */
export function getContainerImageBase(projectRoot?: string): string {
  return `coda-agent-${getInstallSlug(projectRoot)}`;
}
