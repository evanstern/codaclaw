...
```bash
rsync -a .agents/skills/add-vercel/container-skills/ container/skills/
```
...
for session_dir in data/v2-sessions/ag-*; do
  if [ -d "$session_dir/.agents-shared/skills" ]; then
    rsync -a container/skills/ "$session_dir/.agents-shared/skills/"
