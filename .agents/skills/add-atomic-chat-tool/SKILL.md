...
```bash
cp .agents/skills/add-atomic-chat-tool/atomic-chat-mcp-stdio.ts container/agent-runner/src/atomic-chat-mcp-stdio.ts
```
...
```bash
# Atomic Chat MCP tool (.agents/skills/add-atomic-chat-tool)
