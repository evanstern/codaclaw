...
pnpm exec dotenv -e .env -- pnpm exec tsx .agents/skills/x-integration/scripts/setup.ts
...
```bash
echo '{"content":"Test tweet - please ignore"}' | pnpm exec dotenv -e .env -- pnpm exec tsx .agents/skills/x-integration/scripts/post.ts
```

### Test Like

```bash
echo '{"tweetUrl":"https://x.com/user/status/123"}' | pnpm exec dotenv -e .env -- pnpm exec tsx .agents/skills/x-integration/scripts/like.ts
```
...
echo '{"content":"Test"}' | pnpm exec tsx .agents/skills/x-integration/scripts/post.ts
```
